...
import { Edit, Save, X, MapPin, ChevronDown } from "lucide-react";
...
export function ProfileAbout({
  ...
  const handleRegionUpdate = async (newRegion: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || "거래지역 업데이트에 실패했습니다.");
      }
    } catch (error) {
      console.error("거래지역 업데이트 실패:", error);
      toast.error("거래지역 업데이트 중 오류가 발생했습니다.");
    } finally {
              ...
              {isEditingRegion ? (
                <div className="space-y-3">
                  <select
                    ...
                    onChange={e => {
                      if (e.target.value) {
                        handleRegionUpdate(e.target.value);
                      }
                    }}
                    disabled={regionLoading}
                  >

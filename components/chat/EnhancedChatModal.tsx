    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 md:p-4">
      <div className="bg-white md:rounded-lg w-full md:max-w-6xl h-full md:h-[90vh] flex">
        ...
        >
                                                    ...
                                                    <button
                                                      onClick={() => {
                                                        if (
                                                          ...
                                                          toast.success(
                                                            `운송장번호: ${chatData.item.shippingInfo.trackingNumber}\n택배사: ${chatData.item.shippingInfo.courier}`
                                                          );
                                                        } else {
                                                          toast.error(
                                                            "배송정보가 없습니다."
          ...
          {/* 메시지 영역 */}
          <div
            className={`flex-1 overflow-y-auto p-4 space-y-4 pb-safe ${chatData?.tradeType === "안전결제" ? "pt-4" : ""}`}
          >
            {/* 사기 경고 메시지 */}
          ...
          <AnimatePresence>
            {showBottomSheet && chatData && user && (
              <>
                ...
                {/* Bottom Sheet */}
                <motion.div
                  className="fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-2xl z-50 max-w-[480px] mx-auto md:hidden"
                  initial={{ y: "100%" }}
                  animate={{ y: 0 }}
                  exit={{ y: "100%" }}

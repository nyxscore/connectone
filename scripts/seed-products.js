/**
 * 가상 상품 데이터 생성 스크립트
 * Firebase에 테스트용 상품 데이터를 추가합니다.
 *
 * 실행 방법:
 * 1. Firebase Console에서 서비스 계정 키 다운로드
 * 2. 환경변수로 키 파일 경로 설정
 * 3. 스크립트 실행: GOOGLE_APPLICATION_CREDENTIALS=./serviceAccountKey.json node scripts/seed-products.js
 */
...
try {
  ...
  console.log("2. 프로젝트 설정 > 서비스 계정 탭");
  console.log('3. "새 비공개 키 생성" 클릭하여 JSON 파일 다운로드');
  console.log("4. 다운로드한 파일을 serviceAccountKey.json으로 저장");
  console.log("5. 다음 명령어로 실행:");
  console.log(
  ...
  try {
    ...
    for (const seller of sellers) {
      ...
      if (!userDoc.exists) {
        ...
        console.log(`  ✅ ${seller.nickname} 추가 완료`);
      } else {
        console.log(`  ⏭️  ${seller.nickname} 이미 존재함`);
      }
    }
    ...
    for (const product of products) {
      ...
      addedCount++;
      console.log(`  ✅ [${addedCount}/${products.length}] ${product.title}`);
    }
